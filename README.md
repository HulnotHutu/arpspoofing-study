...
本实验程序采用单向、明确目标的方式：周期性向 `victim_ip` 发送 ARP reply，在其中把 `spoofed_ip` 的 MAC 声明为本机 MAC。退出时再用真实的 `spoofed_mac` 向目标发送恢复报文，尽量把目标 ARP 缓存恢复为正确映射。

## 协议字段对应关系

程序主动向 `victim_ip` 发送伪造 ARP reply 时，报文字段为：

```text
Ethernet destination = victim MAC
